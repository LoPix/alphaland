		...
		if (AVS_RefuelSystemActive && AVS_FillCanisterActive && {_vehicle isEqualTo player}) then 
		{
			...
			if (_fillPoints > 0 && (magazines player find "Exile_Item_FuelCanisterEmpty" >= 0)) then 
			{
				if (!AVS_fillActionAdded) then 
				{
					...
					_fillTitle = format ["<t size='1' shadow='1'><t color='#00F600'>Leeren Benzinkanister füllen: %1 Poptabs</t>", _canisterEmptyCost];
				...
				if (_refuelPoints > 0) then
				{
					...
					if (!AVS_refuelActionAdded) then 
					{
						if (!AVS_Refueling) then {
							...
							_title = "<t size='2' shadow='1'><t color='#00F600'>Fahrzeug betanken</t>";
							_description = format ["<t size='1' shadow='1'><t color='#FFFFFF'>Benzinpreis: %1 Poptabs/Liter</t>", AVS_FuelCost];
