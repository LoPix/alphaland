...
ExAd_XM8_DV_fnc_itemsMissing = {
	...
	_recipeStr = "";
	{
		...
		_recipeStr = _recipeStr + format["<br />",(if(_amount < 0)then{((-1) * _amount)}else{_amount}), _text];
	}forEach _recipe;
	
	_response = format["You can only spawn a bike...<br />%1",_recipeStr];
...
ExAd_XM8_DV_fnc_spawnVehicle = {
	...
	{	
		...
		if(_bambiState && !ExileClientPlayerIsBambi) then { throw "You can only spawn bike as a bambi!"};
		...
		[_slideClass] spawn {
			...
			uiSleep 1;
			["SuccessTitleAndText", ["Bike deployed"]] call ExileClient_gui_toaster_addTemplateToast;
		...
		[_exception] spawn {
			UISleep 0.5; 
			["ErrorTitleAndText", ["Whoops - Deploy Bike", _this select 0]] call ExileClient_gui_toaster_addTemplateToast;
			["extraApps", 1] call ExileClient_gui_xm8_slide;
		};
	};
};