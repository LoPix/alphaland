...
_servicePointClasses = ["Land_FuelStation_02_workshop_F","Land_FuelStation_01_workshop_F"]; // service point classes (can be house, vehicle and unit classes)
_maxDistance = 40; // maximum distance from a service point for the options to be shown
...
_actionColour = "#FFB418"; //the colour of the scroll action Blue: "#0096ff"
...
_NoGoWeapCName = [
        //irrelevant ones
        "Horn","MiniCarHorn","SportCarHorn","TruckHorn2","TruckHorn","BikeHorn","CarHorn","TruckHorn3",
        //not allowed ones
        ...
        if (local _vehicle && _inVehicle) then {
                ...
                if (_inRange && (speed (_vehicle) < 1) && (speed (_vehicle) > -1)) then {
                        ...
                        if (SP_repair_action < 0 && _repair_enable) then {
                                ...
                                _actionTitle = [format["Reparieren %1",getText (configFile >> "Cfgvehicles" >> typeOf _vehicle >> "displayName")], _costs] call _fnc_actionTitle;
