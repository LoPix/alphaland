...
if (_exilew < _costs) exitWith {['ErrorTitleAndText',[format['You need %1 Pop tab to Repair %2', _costs,_name]]] call ExileClient_gui_toaster_addTemplateToast;};
...
_allRepaired = true;
{
        ...
        if ((vehicle player != _vehicle) || (!local _vehicle) || ([0,0,0] distance (velocity _vehicle) > 1)) exitWith {
                _allRepaired = false;
                ['ErrorTitleAndText', [format['Repairing of %1 stopped', _name]]] call ExileClient_gui_toaster_addTemplateToast;
        };
        ...
        if (_damage > 0) then {
                if (_repairTime > 0) then {
                        ...
                        ['InfoTitleAndText',[format['Repairing%1 ...', _partName]]] call ExileClient_gui_toaster_addTemplateToast;
                        sleep _repairTime;
                };
...
if (_allRepaired) then {
        ...
        ['SuccessTitleAndText', [format['%1 Repaired', _name]]] call ExileClient_gui_toaster_addTemplateToast;
};
