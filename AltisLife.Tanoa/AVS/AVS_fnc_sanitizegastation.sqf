...
if (AVS_DisableStockRefuel) then
{
	...
	_gastation = nearestobjects [_centermap, ["Land_fs_feed_F","Land_FuelStation_02_pump_F","Land_FuelStation_01_pump_F"], _radiusmap];
	Gastation_locations = [];
	{Gastation_locations = Gastation_locations + [getpos _x]} foreach _gastation;

	{
		_gastationToEmpty = nearestObjects [_x, ["Land_fs_feed_F","Land_FuelStation_02_pump_F","Land_FuelStation_01_pump_F"], 10];
